...
\begin{abstract}
Reasoning systems do not operate on full system state. They operate on compressed representations, where distinctions are reduced or eliminated in order to make inference tractable. This compression may eliminate distinctions that later reasoning requires. When such distinctions remain operationally relevant, the result is representational loss rather than merely uncertainty.

A recurring and structurally distinct class of reasoning failures is representational rather than merely inferential: when a query is not identifiable under the current representation, no improvement in inference over that representation can recover the missing distinction. When systems attempt to reconstruct distinctions that no longer exist, they produce overconfident conclusions, unstable decisions, and explanations that appear coherent but are not grounded in recoverable state.

This paper identifies a structural failure mode across compressed reasoning systems: non-identifiability under projection is misclassified as recoverable inference error. We distinguish between error, which is correctable within the current representation, and loss, which is not identifiable without representation expansion, external information, or additional assumptions. We then outline structural conditions for reasoning that aims to remain reliable under projection: observability of loss-relevant state, bounded correction, and non-collapse of distinct representational levels.
...
When projection removes a distinction that is not otherwise encoded in the representation, that distinction cannot be reconstructed without additional information, an expanded representation, or an explicitly valid reconstruction rule.
...
Reasoning under projection is therefore constrained by the information preserved in the representation. Any attempt to infer distinctions that have been lost must either introduce additional assumptions, expand the representation, or obtain information outside the current representation.
...
Observability of loss is necessarily partial and relative. A reasoning system need not be able to detect every distinction removed by projection; in general, it cannot. The requirement is not complete detection of loss, but preservation of detectable loss conditions relative to the representational language, metadata, or meta-representation available to the system. This includes:
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Unsupported determinacy:} when required distinctions are destroyed, the system does not present a determinate answer as justified by the projection alone (Case 1).
...
In this case, the query ``which action is uniquely best?'' is not identifiable under the projection. However, a decision may still be justified by an explicit rule, such as minimizing worst-case harm, deferring irreversible action, or selecting the option with a bounded fallback path.
...
Reasoning systems operate over representations that compress the underlying structure of the problems they address. This compression is not optional. It is required for tractability. However, it can eliminate distinctions that later reasoning may require.
...
Distinguishing between error and loss provides a structural foundation for reasoning under projection. From this distinction follow three requirements: observability of loss-relevant state, bounded correction, and non-collapse of distinct levels. These conditions do not guarantee correctness, but they constrain how reasoning behaves in the presence of incomplete information.
...
The broader implication is that improvements in scale, optimization, or model complexity are not sufficient to produce stable reasoning on their own. Without structural constraints that account for projection and loss, increased capability may produce systems that are more persuasive without becoming proportionally more trustworthy.

Reasoning under projection is therefore not a rare edge case. It is the typical condition in complex reasoning systems, especially when decisions depend on compressed, partial, or task-shaped representations. Even when a representation is lossless for one query, it may be lossy for another. Recognizing these constraints is a necessary step for systems that aim not merely to produce answers, but to maintain a consistent relationship to the information they actually contain. How those constraints should be implemented or evaluated is a separate question beyond the scope of this paper.
