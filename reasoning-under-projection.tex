...
This paper identifies a structural failure mode across compressed reasoning systems: non-identifiability under projection is misclassified as recoverable inference error. We distinguish between error, which is correctable within the current representation, and loss, which is not identifiable without representation expansion, external information, or additional assumptions. We then outline structural conditions that any reliable reasoning process under projection should respect: observability of loss-relevant state, bounded correction, and non-collapse of distinct representational levels.
...
Reasoning under projection is therefore not a rare edge case. It is the typical condition in complex reasoning systems, especially when decisions depend on compressed, partial, or task-shaped representations. Even when a representation is lossless for one query, it may be lossy for another. Recognizing these constraints is a necessary step toward systems that do not merely produce answers, but maintain a consistent relationship to the information they actually contain. How those constraints should be implemented or evaluated is a separate question beyond the scope of this paper.
