...
\begin{abstract}
Reasoning systems do not operate on full system state. They operate on compressed representations, where projection may collapse distinctions required for later inference, decision, or evaluation. When such distinctions remain operationally relevant and are not preserved or boundedly recoverable, the result is representational loss rather than merely uncertainty or correctable error.
...
This paper identifies a structural failure mode across compressed reasoning systems: non-identifiability under projection is misclassified as recoverable inference error. We distinguish between error, which is correctable within the current representation, and loss, which cannot be resolved within the current representation without representation expansion, external information, or additional assumptions. When such loss cannot be resolved, it must be explicitly marked or traced so that claims depending on the missing distinction are not presented as admissible. We then outline structural conditions for reasoning that aims to remain reliable under projection: observability of loss-relevant state, bounded correction, and non-collapse of distinct representational levels.
...
Consider a system asked to make a high-stakes classification based on a compressed summary of the available evidence. It produces a clear, well-justified answer, such as “approve” or “do not approve,” supported by that summary. The reasoning appears sound. However, the compressed representation may have collapsed distinctions between reversibility, uncertainty, causal structure, and decision criteria. The system has not made an error in reasoning; it has reasoned over a representation in which the information required to justify its conclusion is not preserved or boundedly recoverable.
...
When projection collapses a distinction that is not otherwise encoded in the representation, and that distinction is required for the query at hand, the distinction cannot be reconstructed without additional information, an expanded representation, or an explicitly valid reconstruction rule.
...
Unlike error, loss cannot be corrected through additional reasoning over the same representation alone. Instead, it must be explicitly marked, bounded, or traced. Such marking preserves accountability for the missing distinction, but does not make claims depending on that distinction admissible.
...
The immediate consequences motivate three structural requirements that later work treats as necessary consequences of reconstructibility under projection: observability of loss-relevant state, bounded correction, and non-collapse of distinct representational levels. Unsupported determinacy motivates observability of loss-relevant state: the system must preserve detectable signals that a query is unsupported. The correction boundary motivates bounded correction: the system must not treat hidden reconstruction as ordinary repair. Decision/inference separation motivates non-collapse of distinct representational roles: the system must not treat a projected object as if it had the authority of the richer object from which it was projected.

These requirements are not additional impossibility results. They are structural constraints on reasoning systems that operate under projection.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Expansion of collapsed structure:} when distinctions have been compressed, the system reconstructs a representation that preserves them (Case 2).
    ...
    \item \textbf{Augmentation of missing structure:} when necessary structure is absent, the system introduces monitoring, ownership, or other admissibility-preserving structure (Cases 3 and 4).
...
\begin{itemize}[leftmargin=1.5em]
    \item when the representation fails to preserve or boundedly recover essential distinctions, the system rejects the projection rather than producing an unjustified conclusion.
\end{itemize}
...
A structurally constrained response would handle the task differently. It explicitly identifies the collapse of multiple causal factors into a single attribution as a loss of structure. It then reconstructs a valid representation by separating:
\begin{itemize}[leftmargin=1.5em]
...
This borderline case matters because it prevents the framework from treating every failure as projection loss. The error--loss distinction is useful only if it separates cases where correction within the representation is possible from cases where the representation itself no longer supports the requested distinction.
...
The examples distinguish refusal, restructuring, and augmentation as possible responses to non-identifiability, but this paper does not provide a complete policy for selecting among them. That choice depends on whether representation expansion is available, whether additional observations can be obtained, whether the task permits abstention, and whether a decision rule under non-identifiability has been specified.

Across these examples, respecting the constraints changes whether the system answers, refuses, restructures, augments, or classifies the failure. These behaviors are testable expectations for future work and illustrate how maintaining the error--loss distinction changes the type of reasoning performed under projection.

\section{Relation to Existing Frameworks}

The argument in this paper overlaps with several established lines of work. In statistics and causal inference, identifiability asks whether a target quantity can be determined from the available observational structure \cite{Pearl2009}. In partially observable decision processes, agents must act over compressed or incomplete state representations \cite{Kaelbling1998}. In formal methods and abstract interpretation, abstraction maps concrete states into reduced domains while preserving selected properties \cite{Cousot1977}. In information-theoretic approaches such as the information bottleneck, compressed representations are evaluated by what task-relevant information they retain \cite{Tishby1999}.

The present paper does not claim to replace these frameworks. Its contribution is to identify a behavioral failure mode that cuts across them. Prior work defines when a query is identifiable, when an abstraction preserves a property, or when an agent acts under partial observability. The failure analyzed here occurs when a reasoning system behaves as though the relevant identifiability or preservation condition holds when it does not. In that case, non-identifiability under projection is misclassified as ordinary inference error, and the system produces determinate outputs from a representation that does not support them.

The formal ingredients used here are not new in isolation. Identifiability, partial observability, abstraction, and underdetermination all describe cases in which a representation fails to preserve distinctions relevant to a query or decision. The contribution of this paper is not to replace those accounts, but to identify a recurring reasoning-system failure that cuts across them: systems often behave as though the relevant identifiability, sufficiency, or preservation condition holds when it does not. This misclassification turns representational insufficiency into apparent inference error, leading systems to repair, explain, or decide beyond what the current representation supports.

In a POMDP, partial observability is typically handled by maintaining or updating a belief state. In abstract interpretation, loss is managed through sound abstraction and possible refinement. In causal inference, non-identifiability blocks recovery of a target from the available observational structure. The present paper abstracts over these settings and focuses on a different question: how should a reasoning system behave when its current representation does not support the query being asked? The error--loss distinction is therefore a diagnostic constraint on reasoning behavior, not a replacement for domain-specific theories of identifiability, abstraction, or belief-state update.

The contribution is not a new theory of identifiability, abstraction, or partial observability. It is a unifying account of a reasoning failure mode: systems produce determinate outputs as though the relevant identifiability or preservation condition holds when the current representation does not support it.

\section{Why Existing Approaches May Miss Projection Loss}
...
Without explicit representation of projection loss, evaluation over outputs in $R$ cannot reliably distinguish between:
\begin{itemize}[leftmargin=1.5em]
...
This paper focuses on structural behavior rather than specific implementations. Operational instantiations, scaffold designs, evaluation harnesses, and empirical results are therefore outside its scope.
...
Although this paper does not propose a specific implementation, the structural requirements suggest several implementation hooks.
...
Second, systems can track whether a requested query is supported by the current representation, distinguishing identifiable questions from questions that require additional information.

Third, systems can use abstention or refusal criteria when a determinate answer would require resolving distinctions not present in the representation.
...
These hooks do not constitute a complete architecture. They indicate how the structural account could be made operational without committing this paper to a particular scaffold, runtime system, or evaluation method.
